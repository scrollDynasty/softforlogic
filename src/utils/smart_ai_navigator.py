    ...
    async def _analyze_current_state_internal(self, page: Page) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                screenshot = await asyncio.wait_for(page.screenshot(), timeout=3.0)
                logger.info("📸 Шаг 3: Скриншот получен, кодирую в base64...")
                screenshot_b64 = base64.b64encode(screenshot).decode()
                logger.info(f"✅ Скриншот получен: {len(screenshot_b64)} символов")
            except asyncio.TimeoutError:
                logger.warning("⏰ ТАЙМАУТ: Не удалось получить скриншот за 3 секунды, продолжаю без скриншота")
                screenshot_b64 = ""
            except Exception as e:
                logger.warning(f"⚠️ Ошибка получения скриншота: {e}, продолжаю без скриншота")
                screenshot_b64 = ""
            ...
            """), timeout=5.0)
                ...
                logger.warning("⏰ ТАЙМАУТ: Не удалось получить информацию о странице за 5 секунд")
                logger.info("🔄 Использую альтернативный метод получения информации...")
                page_info = None
            except Exception as e:
                logger.warning(f"⚠️ Ошибка получения информации о странице: {e}")
                logger.info("🔄 Использую альтернативный метод получения информации...")
                page_info = None
            
            # Если JavaScript не сработал, используем базовую информацию
